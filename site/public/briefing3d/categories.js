...
(function () {
  ...
  /* ---------------------------------------------------------------- the Union Flag, drawn on a canvas
  ...
  function flagTexture() {
    ...
    const g = c.getContext("2d");
    // Painted onto a dark backdrop at reduced alpha, so the flag reads as lit from within, like the topic
    // spheres, rather than as a flat decal. The Union Flag fills the whole texture on this edition.
    g.fillStyle = "#040918"; g.fillRect(0, 0, W, H);
    g.globalAlpha = 0.85;
    drawUnionJack(g, 0, 0, W, H);
    g.globalAlpha = 1;
  ...
  function makeOverallSphere(node) {
    ...
    // The Union Flag, inside a faceted containment shell, with a rim-lit atmosphere and a fixed
  ...
  function renderPanel(highlightMetric) {
    ...
    panel.appendChild(h("div", "p-kicker", n.type === "overall" ? "Every measure tracked here" : `Topic · ${n.metrics.length} measure${n.metrics.length === 1 ? "" : "s"}`));
    ...
    typewrite(plain, n.type === "overall"
      ? "Every measure tracked here, rolled into one figure. Click a sphere on the ring to open a topic."
      : n.blurb);
    const row = h("div", "p-stat-row");
    row.append(statChip(n.off, "Worse", "fail"), statChip(n.watch, "Mixed", "warn"), statChip(n.on, "Better", "pass"));
    panel.appendChild(row);
    panel.appendChild(h("div", "p-h", n.type === "overall" ? "Jump to a topic" : "Measures in this topic"));
    if (n.type === "overall") {
    ...
    const a1 = h("a", "btn-hud", "Open the full record ↗");
  ...
  /* ---------------------------------------------------------------- guided tour */
  const tourSteps = [{ id: "overall", say: `This site tracks ${S.metrics_total} measures of government performance. Since the government took office in May 2022, ${S.metrics_off_track} of ${S.metrics_rated} rated measures are worse or off track, and ${S.metrics_on_track} are better or on track.` }]
    .concat(sections.map((s) => ({ id: s.id, say: `${s.title}: ${s.off} of ${s.rated} rated measures are worse or off track since May 2022. ${s.blurb}` })))
    .concat([{ id: "overall", say: "That's every topic. Click any sphere to explore it, or open a measure to see its source." }]);
