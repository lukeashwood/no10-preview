...
export const EDITORIAL: Record<string, Editorial> = {
  ...
  clean_power: {
    ...
    target: {
      ...
      rule: 'Judged on the latest official clean-power share against the straight-line path from the 2024 starting point to 95% in 2030: on track if at or above that path, at risk within 5 percentage points, off track below that.',
      rate: (m) => {
        ...
        const txt = `${m.headline.value}% of electricity was clean in ${year}; the straight-line path to 95% by 2030 implies about ${path.toFixed(1)}% by then.`;
